...
public class GlobalExceptionHandlingMiddleware
{
    ...
    //TODO: Simple options to make the response more readable, might on future be on config
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        {
            if (ex is not ArgumentException) _logger.LogTrace(ex, "An unhandled exception occurred during request processing");

            await HandleExceptionAsync(context, ex);
        }
    }

    // TODO: Move this to shared domain project, not sure how to handle it right now but it works fine.
    /// <summary>
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        ...
        var errorResponse = exception switch
        {
            ...
            ApplicationException applicationException => CreateErrorResponse(
                "Application Error",
                HttpStatusCode.BadRequest,
        ...
        switch (exception)
        {
            case UserNotFoundException:
                _logger.LogTrace(exception, "Information not found: {Message} | Path: {Path}", exception.Message, context.Request.Path);
                break;
            case ADomainException:
                _logger.LogWarning("Domain validation error: {Message} | Path: {Path}", exception.Message, context.Request.Path);
                _logger.LogTrace(exception, "Domain validation error: {Message} | Path: {Path}", exception.Message, context.Request.Path);
                break;
            case ApplicationException:
                _logger.LogError(exception, "Application error: {Message} | Path: {Path}", exception.Message, context.Request.Path);
                break;
            case ArgumentException:
                _logger.LogTrace(exception, "Invalid argument: {Message} | Path: {Path}", exception.Message, context.Request.Path);
                _logger.LogWarning("Invalid argument:{Message} | Path: {Path}", exception.Message, context.Request.Path);
                break;
            case TimeoutException:
                _logger.LogError(exception, "Operation timed out");
                break;
            case UnauthorizedAccessException:
                _logger.LogTrace(exception, "Unauthorized access attempt: {Message} | Path: {Path}", exception.Message, context.Request.Path);
                _logger.LogWarning("Unauthorized access attempt: {Message} | Path: {Path}", exception.Message, context.Request.Path);
                break;
            default:
                _logger.LogError(exception, "Unexpected error: {Message} | Path: {Path}", exception.Message, context.Request.Path);
                break;
        }
